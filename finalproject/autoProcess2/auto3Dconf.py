...
def transProc(numList, inputFiles):
    numList.reverse()
    inputFiles.reverse()

    # Extract and create common pass point files
    extractCommonPt(numList, inputFiles)

    # Preform 3D conformal transformation for all models
    for i in range(len(numList)):
        # Read 3D points from two model files
        model1 = open("_" + inputFiles[i])
        lines1 = model1.readlines()
        model1.close()

        # Transform right model points to left model
        # Get points whose coordinates are to be transformed
        data = np.array(map(
            lambda l: l.split(),
            lines1[1:-numList[i]])).astype(np.double)
        x, y, z = map(lambda x: x.flatten(), np.hsplit(data, 3))

        controlPtFile = (
            "common%d%d.txt" % ((len(numList)+1-i), (len(numList)-i)))

        X, Y, Z = trans3D(x, y, z, controlPtFile)

        # Write out the result to the next model point file
        model2 = open("_" + inputFiles[i + 1])
        lines2 = model2.readlines()
        model2.close()

        # Replace original left model file
        dest = open("_" + inputFiles[i + 1], "w")
        dest.write("".join(lines2[:2]))     # Exposure station of left model

        for j in range(len(X)):
            # Point transformed from right model
            dest.write("%.8f %.8f %.8f\n" % (X[j], Y[j], Z[j]))

        # Common control points from left model
        dest.write("".join(lines2[-numList[i]:]))

        # Control points of the next transform series
        dest.write("".join(lines2[2:-numList[i]]))
        dest.close()

    # Rename result point file name
    shutil.move("_" + inputFiles[i + 1], "result.txt")

    # Remove template files
    [os.remove("_" + f) for f in inputFiles[:-1]]


def extractCommonPt(numList, inputFiles):
    """Extract common 3D pass point from all models"""

    # Create template files
    [shutil.copy(f, "_" + f) for f in inputFiles]

    for i in range(len(numList)):
        # Read 3D points from two model files
        model1 = open("_" + inputFiles[i])
        lines1 = map(lambda l: l.replace("\n", " "), model1.readlines())
        model1.close()
        model2 = open("_" + inputFiles[i + 1])
        lines2 = map(lambda l: l.replace("\n", " "), model2.readlines())
        model2.close()

        # Extract common control point information
        common = open("common%d%d.txt" % (
            (len(numList)+1-i), (len(numList)-i)), "w")
        # For left expore station
        common.write((lines1[0]+lines2[1]+"\n"))

        # For common pass points
        if i == 0:
            common.write("\n".join(map(
                lambda (l, r): l + r,
                zip(lines1[-numList[i]:], lines2[-numList[i]:]))))
        else:
            common.write("\n".join(map(
                lambda (l, r): l + r,
                zip(lines1[-numList[i] - numList[i - 1]:-numList[i - 1]],
                    lines2[-numList[i]:]))))


def trans3D(xp, yp, zp, controlPtFile):
    """Perform 3D conformal transformation"""
    # Read control point coordinates from file
    ...
    X = S0 * (M[0, 0]*xp + M[1, 0]*yp + M[2, 0]*zp) + Tx0
    Y = S0 * (M[0, 1]*xp + M[1, 1]*yp + M[2, 1]*zp) + Ty0
    Z = S0 * (M[0, 2]*xp + M[1, 2]*yp + M[2, 2]*zp) + Tz0
...
def main():
    numList = [20, 18, 25]
    inputFiles = ["model12.out", "model23.out", "model34.out", "model45.out"]
    transProc(numList, inputFiles)
